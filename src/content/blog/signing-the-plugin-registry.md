...
Before a plugin is *enabled* (at every arming point: installing from the
registry, installing from a file, or flipping the per-plugin checkbox in
Settings), Editora shows a **capability disclosure**. It answers, in plain terms:
does this plugin ship a jar, meaning it runs arbitrary code? Does it declare
external commands, and with what arguments? Does it remap any keybindings? You
opt in per plugin, with that information in front of you, rather than trusting a
name.
...
- **The right bytes.** The registry entry carries a sha-256, and install
  **verifies it**, so a mismatch aborts. Fetches are HTTPS-only with no
